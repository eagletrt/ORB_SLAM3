...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const cv::Mat &orbMask, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
    ExtractORB(0,imGray,orbMask,0,0);
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndExtORB = std::chrono::steady_clock::now();
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const cv::Mat &ma, const int x0, const int x1)
{
    vector<int> vLapping = {x0,x1};
    if(flag==0)
        monoLeft = (*mpORBextractorLeft)(im,ma,mvKeys,mDescriptors,vLapping);
    else
        monoRight = (*mpORBextractorRight)(im,ma,mvKeysRight,mDescriptorsRight,vLapping);
}
